...
int main(){
    ...
    Fraction f3 = ++(++f1);
    // here value of f3 is 7/1 and also value of f1 is 7/1, if we had not used "&" so it would have made copies and the updated value will not have showd for f1 it would only have shown to f3.
    f1.Print();
    f3.Print();
    return 0;
}